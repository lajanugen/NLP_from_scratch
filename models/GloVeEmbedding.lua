...
function GloVeEmbedding:__init(word2idx, embedding_size, data_dir)
	local embedding_file = 'models/glove.840B.300d.txt'
	local vocab_embedding_file = 'wsj'
    local file_embedding_size = 300
    self.vocab_size = numkeys(word2idx)
    self.word2idx = word2idx
    parent.__init(self, self.vocab_size+1, embedding_size) -- +2 to incorporate START and STOP symbols
    print("loading glove vectors")
    self.embedding_size = embedding_size
    --load glove vectors as a tensor from a .t7 file if it exists, otherwise generate that .t7 file
    local vocab_embedding_file = path.join(data_dir, "wsj_" .. self.vocab_size .. "x" .. embedding_size .. ".t7")
...
function GloVeEmbedding:parseEmbeddingFile(embedding_file, file_embedding_size, word2idx)
    ...
    local weight = torch.Tensor(self.vocab_size+2, file_embedding_size) -- +2 to incorporate START and STOP symbols
    ...
    for line in io.lines(embedding_file) do
		...
		--assert(#parts == file_embedding_size, #parts)
        local word = parts[1]
        if word_lower2idx[word] then
            local idx = word_lower2idx[word]
            for i=2, #parts do
				--print(idx,i-1)
				--print(weight[idx][i-1])
				--print(idx,i-1,#parts)
